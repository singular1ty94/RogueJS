...
class RogueJS {
  ...
  init() {
    ...
    //Bind the displays
    document
      .getElementById("RogueCanvas")
      .appendChild(this.display.getContainer());
    document.getElementById("RogueHUD").appendChild(this.hud.getContainer());
    document
      .getElementById("RogueMessages")
      .appendChild(this.msgLog.getContainer());

    document
      .getElementById("RogueCanvas")
      .addEventListener("click", canvasClick);

    //The fov
    this.fov = new ROT.FOV.PreciseShadowcasting(lightPasses);
    this.lighting = new ROT.Lighting(reflectivity, {
      range: this.FOV_RADIUS + 1,
      passes: 1
    });
  ...
  createMonsters(level) {
    ...
    while (mobsPlaced < mobsToPlace) {
      for (var i = 0, len = monsters.length; i < len; i++) {
        ...
        if (
          !place &&
          monster.weighting.rare &&
          (level >= monster.weighting.rare[0] &&
            level <= monster.weighting.rare[1])
        ) {
        ...
        if (
          !place &&
          monster.weighting.uncommon &&
          (level >= monster.weighting.uncommon[0] &&
            level <= monster.weighting.uncommon[1])
        ) {
        ...
        if (
          !place &&
          monster.weighting.common &&
          (level >= monster.weighting.common[0] &&
            level <= monster.weighting.common[1])
        ) {
          if (chance <= CHANCE_COMMON) {
            place = true;
          }
        }
        if (
          !place &&
          monster.weighting.frequent &&
          (level >= monster.weighting.frequent[0] &&
            level <= monster.weighting.frequent[1])
        ) {
          if (chance <= CHANCE_FREQUENT) {
            place = true;
          }
        }
    ...
    while (itemsPlaced < itemsToPlace) {
      for (var i = 0, len = items.length; i < len; i++) {
        ...
        if (
          !place &&
          item.weighting.rare &&
          (level >= item.weighting.rare[0] && level <= item.weighting.rare[1])
        ) {
        ...
        if (
          !place &&
          item.weighting.uncommon &&
          (level >= item.weighting.uncommon[0] &&
            level <= item.weighting.uncommon[1])
        ) {
        ...
        if (
          !place &&
          item.weighting.common &&
          (level >= item.weighting.common[0] &&
            level <= item.weighting.common[1])
        ) {
        ...
        if (
          !place &&
          item.weighting.frequent &&
          (level >= item.weighting.frequent[0] &&
            level <= item.weighting.frequent[1])
        ) {
  ...
  makeLevel(level) {
    ...
    this.map = new ROT.Map.Uniform(this.w, this.h, {
      ...
      roomDugPercentage: 0.9 /* we stop after this percentage of level area has been dug out */,
...
function recalculateMap() {
  if (RogueJS.player) {
    ...
    for (var y = 0; y < RogueJS.h; y++) {
      for (var x = 0; x < RogueJS.w; x++) {
        ...
        if (RogueJS.discovered[x + "," + y] == 0) {
          ...
          RogueJS.display.draw(
            x,
            y,
            "",
            Colors.DISCOVERED_FLOOR,
            Colors.DISCOVERED_FLOOR
          );
        }
      }
    ...
    RogueJS.lighting.setLight(RogueJS.player.getX(), RogueJS.player.getY(), [
      240,
      240,
      30
    ]);
    ...
    if (RogueJS.player) {
      RogueJS.fov.compute(
        ...
        function(x, y, r, visibility) {
          ...
          var baseColor = RogueJSData[x + "," + y]
            ? [100, 100, 100]
            : [50, 50, 50];
          var light = ambientLight;

          if ([`${x},${y}`] in RogueJSLight) {
...
function drawBar(
  ...
  for (var i = 0; i < displayAmt; i++) {
    ...
    if (i >= startString) {
      titleFormatted +=
        "%c{white}%b{" + colorFore + "}" + title.charAt(i - startString);
    }
  }
  for (var i = displayAmt; i < width; i++) {
    ...
    if (i >= startString) {
      titleFormatted +=
        "%c{white}%b{" + colorBack + "}" + title.charAt(i - startString);
    }
  }
...
function IsOccupied(tileX, tileY) {
  if (
    RogueJS.player &&
    RogueJS.player.getX() == tileX &&
    RogueJS.player.getY() == tileY
  ) {
    return true;
  }
  for (var i = 0; i < Entities.length; i++) {
...
function GetEnemyAtTile(tileX, tileY) {
  for (var i = 0; i < Entities.length; i++) {
    if (
      Entities[i]._x == tileX &&
      Entities[i]._y == tileY &&
      (Entities[i] instanceof Actor || Entities[i] instanceof Player)
    ) {
      return Entities[i];
    }
  }
...
function attackTile(attacker, tileX, tileY) {
  ...
  if (IsOccupied(tileX, tileY)) {
    ...
    if (
      ...
      RogueJS.player.hasPassive(PASSIVE_WEAPONS_RANK_TWO)
    ) {
      damageDealt -= Math.round(damageDealt / 3);
    }

    if (
      defender instanceof Actor &&
      defender.getHP() <= Math.round(defender.getMaxHP() / 3)
    ) {
...
function bloomEffect(tileX, tileY, radius, colour) {
  ...
  const bloomTiles = getCircle(tileX, tileY, radius);
...
function UpdateHUD() {
  ...
  if (RogueJS.player) {
    const curHealth =
      "HP (" + RogueJS.player.getHP() + "/" + RogueJS.player.getMaxHP() + ")";
    const curHealthWidth = curHealth.length + 2;
    drawBar(
      1,
      0,
      curHealthWidth,
      RogueJS.player.getMaxHP(),
      RogueJS.player.getHP(),
      Colors.HEALTH_LIGHT,
      Colors.HEALTH_DARK,
      curHealth
    );
    ...
    const curXP =
      "XP (" +
      RogueJS.player.getXP() +
      "/" +
      RogueJS.player.getNextXP() +
      ") | Lv " +
      RogueJS.player.getLevel();
    const curXPWidth = curXP.length + 2;
    drawBar(
      curHealthWidth + 2,
      0,
      curXPWidth,
    ...
    const curWeapon =
      RogueJS.player._weapon.getName() +
      " " +
      "(" +
      RogueJS.player._weapon.getDamage() +
      " dmg)";
    const curWeaponWidth = curWeapon.length + 2;
    drawBar(
      curHealthWidth + curXPWidth + 3,
      0,
      curWeaponWidth,
      1,
      1,
...
function lightPasses(x, y) {
  var key = x + "," + y;
  if (key in RogueJSData) {
    return RogueJSData[key] == 0;
  }
  return false;
}
